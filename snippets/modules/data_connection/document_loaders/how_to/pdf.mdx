...
这涵盖了如何将在线 PDF 加载到我们可以在下游使用的文档格式中。这可用于各种在线 PDF 网站，如 https://open.umn.edu/opentextbooks/textbooks/和 https://arxiv.org/archive/
...
此外，您可以将 [PyMuPDF 文档](https://pymupdf.readthedocs.io/en/latest/app1.html#plain-text/) 中的任何选项作为关键字参数传递给 `load` 调用，并将其传递给 `get_text()` 调用。
