...
以下是创建具有自定义错误消息的自定义审核链的示例。它需要对OpenAI的审核终端结果有一些了解（[请参阅此处的文档](https://beta.openai.com/docs/api-reference/moderations)）。
...
要将审核链与LLMChain轻松组合在一起，您可以使用SequentialChain抽象。

让我们从一个简单的例子开始，LLMChain只有一个输入。为此，我们将提示模型说一些有害的内容。
...
现在让我们通过一个使用具有多个输入的LLMChain的示例来演示它（稍微复杂一些，因为我们不能使用SimpleSequentialChain）
