...
使用 `map_reduce` 链时，需要记住的一个问题是在映射步骤中使用的批次大小。如果太大，可能会导致速率限制错误。您可以通过设置所使用的 LLM 上的批次大小来控制此参数。请注意，这仅适用于具有此参数的 LLM。下面是一个示例：
...
我们还可以返回 `refine` 链的中间步骤，以便检查它们。这可以通过设置 `return_refine_steps` 变量来实现。
