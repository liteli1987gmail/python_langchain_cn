## 准备数据
首先我们准备数据。在这个例子中，我们从一个长文档中创建多个文档，但是这些文档可以以任何方式获取（这个笔记本的重点是强调在获取文档之后要做什么）。
...
如果您希望更多地控制和了解正在发生的事情，请参阅下面的信息。

## `stuff` 链

本节展示了使用 `stuff` 链进行摘要的结果。
...
我们还可以返回 `map_reduce` 链的中间步骤，以便进行检查。这是通过 `return_map_steps` 变量完成的。
...
您还可以使用多输入的提示。在这个例子中，我们将使用 MapReduce 链来回答关于我们的代码的特定问题。
...
我们还可以返回 `refine` 链的中间步骤，以便进行检查。这是通过 `return_refine_steps` 变量完成的。
