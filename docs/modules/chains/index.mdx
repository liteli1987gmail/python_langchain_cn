...
在独立使用 LLM 进行简单应用时是可以的，
但更复杂的应用需要将 LLM 进行链式调用 - 可以是与其他 LLM 进行链式调用，也可以是与其他组件进行链式调用。

LangChain 为这种“链式”应用提供了 **Chain** 接口。我们将 Chain 定义得非常通用，作为一个调用组件的序列，其中可以包含其他链。基本接口很简单:
